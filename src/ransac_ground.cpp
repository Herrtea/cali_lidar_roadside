...
int  main(int argc, char *argv[])
{
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/guoz/calibration_ws/src/all527.pcd", *cloud_low) == -1)
    {
        PCL_ERROR("Error loading cloud.pcd.\n");
        return -1;
    }

    // if (pcl::io::loadPCDFile<pcl::PointXYZ>("/home/guoz/calibration_ws/high1291.pcd", *cloud_high) == -1)
    // {
    //     PCL_ERROR("Error loading cloud.pcd.\n");
    //     return -1;
    // }
    
    *cloud = *cloud_high + *cloud_low; 
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_filtered(new pcl::PointCloud<pcl::PointXYZ>); //存储平面点云
    ...
    extract.setIndices(inliers_ptr);                                                 //设置分割后的内点为需要提取的点集
    extract.setNegative(true);                                                      // false提取内点, true提取外点
    extract.filter(*cloud_filtered);                                                //提取输出存储到c_plane
    ...
    viewer->addPointCloud<pcl::PointXYZ>(cloud_filtered, "filtered");
    viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1, 1, 1, "filtered"); // white
